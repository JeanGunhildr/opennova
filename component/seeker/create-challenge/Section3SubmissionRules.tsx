...
export default function Section3SubmissionRules({ rules, onRules }: Section3Props) {
  const newRowRefs = useRef<Record<string, HTMLInputElement | null>>({});

  function addRule() {
    const id = Date.now().toString();
    onRules([...rules, { id, text: "" }]);
  ...
  function updateRule(id: string, text: string) {
    onRules(rules.map(r => r.id === id ? { ...r, text } : r));
  }
  ...
  return (
    <div style={CARD_STYLE}>
      <div className="flex items-start gap-[10px] mb-4">
        ...
        <div>
          <h3 style={{ fontSize: "17px", fontWeight: 700, color: "#F7F7F7" }}>Ketentuan Pengumpulan</h3>
          <p style={{ fontSize: "11px", color: "#737373", marginTop: "3px" }}>Format berkas, dokumen yang wajib dikumpulkan, serta persyaratan submission.</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          {rules.map(rule => (
            <div key={rule.id} className="grid gap-2 items-center" style={{ gridTemplateColumns: "minmax(0,1fr) 34px" }}>
              <input ref={el => { newRowRefs.current[rule.id] = el; }} type="text" value={rule.text}
                ...
                placeholder="Tuliskan ketentuan..." style={INPUT_STYLE} />
              <button type="button" onClick={() => removeRule(rule.id)} disabled={rules.length <= 1}
                ...
                onMouseEnter={e => { if (rules.length > 1) { (e.currentTarget as HTMLElement).style.background = "#3B1313"; (e.currentTarget as HTMLElement).style.borderColor = "#E30000"; (e.currentTarget as HTMLElement).style.color = "#E30000"; } }}
        ...
        <button type="button" onClick={addRule}
          ...
          <Plus size={13} strokeWidth={2.2} />Tambah Ketentuan
        </button>
      </div>
    </div>
  );
